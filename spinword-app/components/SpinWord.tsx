...
const NAV: Array<[View, string]> = [["PLAY", "Play"], ["DAILY", "Daily Challenge"], ["LEADERBOARDS", "Leaderboards"], ["REWARDS", "Rewards"], ["PROFILE", "Profile"], ["FAIRNESS", "Fairness"], ["RULES", "Game Rules"]];
  ...
  return (
    <div className="app-shell">
      ...
      <header className="topbar">
        <button className="brand" onClick={() => navigate("PLAY")} aria-label="SpinWord home"><Image src="/logo.png" alt="SpinWord" width={230} height={66} priority /></button>
        ...
        <div className="casino-minimums">
          <span>MIN PLAY <b>$1</b></span>
          <button className="deposit-button" onClick={() => navigate("REWARDS")}><WalletCards size={15} /><span>DEPOSIT</span><b>$10 MIN</b></button>
          <button className={`header-claim ${session.dailyClaim.available ? "ready" : "claimed"}`} disabled={busy || !session.dailyClaim.available} onClick={claim}><Coins size={15} /><span>{session.dailyClaim.available ? "CLAIM FREE" : "LOOT CLAIMED"}</span><b>{session.dailyClaim.available ? "1M LOOT" : "✓"}</b></button>
        </div>
      ...
      <main className={view === "PLAY" ? "page-wrap play-floor" : "page-wrap"}>
      ...
      {resultOpen && round?.status === "SETTLED" && <ResultModal round={round} onClose={() => setResultOpen(false)} onPlayAgain={() => { setRound(null); setResultOpen(false); setConfirmOpen(true); }}
        onSwitch={() => { setRound(null); setMode(round.mode === "MAX" ? "STANDARD" : "MAX"); setResultOpen(false); }} onHistory={() => { setResultOpen(false); window.setTimeout(() => document.querySelector(".inline-recent")?.scrollIntoView({ behavior: "smooth" }), 0); }} onFairness={() => { setResultOpen(false); navigate("FAIRNESS"); }} />}
    </div>
  );
}
...
function PlayView(props: {
  ...
  return <>
    ...
    <InlineRecentGames refreshKey={`${round?.id ?? "none"}:${round?.status ?? "idle"}:${round?.guessesUsed ?? 0}`} />
  </>;
}

function InlineRecentGames({ refreshKey }: { refreshKey: string }) {
  const [data, setData] = useState<{ rounds: Round[]; total: number } | null>(null);
  const [selected, setSelected] = useState<Round | null>(null);
  useEffect(() => {
    let active = true;
    void api<{ rounds: Round[]; total: number }>("/api/game", { method: "POST", body: JSON.stringify({ action: "recent", limit: 12 }) })
      .then((next) => { if (active) setData(next); })
      .catch(() => { if (active) setData({ rounds: [], total: 0 }); });
    return () => { active = false; };
  }, [refreshKey]);
  return <section className="inline-recent"><div className="recent-strip-head"><div><span className="live-dot" /><span className="eyebrow">LIVE GAME FEED</span><h2>Recent Games</h2></div><span>{data?.total ?? 0} rounds played</span></div><div className="recent-reel">{data?.rounds.length ? data.rounds.map((item) => { const net = (item.payoutAmountMinor ?? 0) - item.playAmountMinor; return <button className={`recent-chip ${item.outcome === "WON" ? "win" : item.status === "ACTIVE" ? "active" : "loss"}`} key={item.id} onClick={() => setSelected(item)}><span>{item.status === "ACTIVE" ? "LIVE" : item.outcome === "WON" ? "WIN" : "DONE"}</span><strong>{item.mode === "MAX" ? "MAX" : "CLASSIC"} · {money(item.playAmountMinor)}</strong><small>{item.status === "ACTIVE" ? `${6 - item.guessesUsed} spins left` : `${multiplier(item.multiplierBasisPoints)} · ${net >= 0 ? "+" : ""}${money(net)}`}</small><i>#{item.publicRef.slice(0, 6)}</i></button>; }) : <div className="recent-empty"><History /><div><strong>Your game feed starts here</strong><span>Start a word and every round will roll into this live casino strip.</span></div></div>}</div>{selected && <RoundReview round={selected} onClose={() => setSelected(null)} />}</section>;
}
