...
export default function SpinWord() {
  ...
  const refresh = useCallback(async () => {
    try {
      const [nextSession, nextProgress] = await Promise.all([api<Session>("/api/session"), api<Progress>("/api/player/daily-progress")]);
  ...
  async function claim() {
    ...
    try { await api("/api/claim/daily-loot", { method: "POST", body: JSON.stringify({ idempotencyKey: crypto.randomUUID() }) }); await refresh(); }
  ...
  async function startRound() {
    setBusy(true); setError("");
    try {
      const created = await api<Round>("/api/rounds", { method: "POST", body: JSON.stringify({ mode, currency, amount, clientSeed: crypto.randomUUID() }) });
  ...
  return <>
    ...
    <section className="game-grid">
      ...
      <section className="center-stack">
        ...
        <GameBoard round={round} busy={props.busy} onRound={props.onRound} onError={props.onError} onResult={props.onResult} />
      </section>
...
function GameBoard({ round, busy, onRound, onError, onResult }: { round: Round | null; busy: boolean; onRound: (round: Round) => void; onError: (error: string) => void; onResult: () => void }) {
  ...
  const submit = useCallback(async () => {
    ...
    try { const updated = await api<Round>(`/api/rounds/${round.id}/guesses`, { method: "POST", body: JSON.stringify({ guess: current }) }); setCurrent(""); onRound(updated); if (updated.status === "SETTLED") onResult(); }
