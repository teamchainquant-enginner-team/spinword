...
describe("server-authoritative gameplay", () => {
  ...
  it("accepts every alphabetic five-letter entry without a dictionary gate", () => {
    const round = createRound(db, playerId, { mode: "STANDARD", currency: "LOOT_COIN", amountMinor: 100 });
    const updated = submitGuess(db, playerId, round.id, "ZZZZZ");
    expect(updated.guesses).toHaveLength(1);
    expect(updated.guesses[0].word).toBe("ZZZZZ");
    expect(() => submitGuess(db, playerId, round.id, "FOUR")).toThrow(/five-letter/i);
  });
