...
import { ACTIVE_POOL_VERSION, DICTIONARY_VERSION, limits, PAYTABLES } from "./config";
...
export function submitGuess(db: SpinwordDb, playerId: string, roundId: string, rawGuess: string): PublicRound {
  return db.transaction(() => {
    ...
    const accepted = db.prepare("SELECT 1 FROM accepted_guesses WHERE dictionary_version = ? AND word = ? AND status = 'APPROVED'").get(DICTIONARY_VERSION, guess);
    if (!accepted) throw new DomainError("UNKNOWN_WORD", "That word is not in the accepted development dictionary.");
