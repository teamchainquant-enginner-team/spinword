...
|---|---|---|
| `POST` | `/api/game` | Unified Vercel-safe bootstrap, claim, round creation, and guess actions |
...
For a Vercel sandbox preview, leave `SPINWORD_DB_PATH` unset. The app detects
Vercel and uses its writable `/tmp` directory instead of the read-only deployment
bundle. This fixes preview execution but the data is ephemeral and may reset or
differ between function instances; it is not a durable production database.
Interactive gameplay uses the unified `/api/game` function so starting a round
and checking its guesses share the same sandbox function state.
...
- `SPINWORD_DB_PATH`: durable SQLite file for local/single-instance hosting; leave unset for a Vercel sandbox preview.
...
- `SPINWORD_SANDBOX_AUTH`: sandbox auto-player; disable when real auth is wired.
